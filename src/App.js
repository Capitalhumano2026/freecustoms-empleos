...
async function guardarPerfil(e) {
  ...
  function PantallaVacantes() {
    return (
      <div>
        <input placeholder="Buscar vacantes..." style={{width:'100%',padding:'9px 12px',borderRadius:8,border:'0.5px solid var(--color-border-secondary)',fontSize:13,marginBottom:12,fontFamily:'inherit',background:'var(--color-background-primary)',color:'var(--color-text-primary)'}} />
        ...
        {filtradas.map(v=>(
          <div key={v.id} className="card" style={{cursor:'pointer'}} onClick={()=>setDetail({type:'vacante',data:v})}>
            ...
            <p style={{fontSize:12,color:'var(--color-text-secondary)',lineHeight:1.5}}>{v.descripcion?.substring(0,90)}…</p>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:10}}>
              <span style={{fontSize:11,color:'var(--color-text-secondary)'}}>{v.postulantes} postulantes</span>
  ...
  function PantallaVacanteDetalle({ v }) {
    ...
    return (
      <div>
        ...
        <p style={{fontSize:13,lineHeight:1.65,color:'var(--color-text-secondary)',marginBottom:14}}>{v.descripcion}</p>
        <p className="section-title">Requisitos</p>
        <ul style={{listStyle:'none',marginBottom:16}}>
          {(v.requisitos||[]).map((r,i)=><li key={i} style={{fontSize:13,padding:'5px 0',display:'flex',alignItems:'center',gap:8,borderBottom:'0.5px solid var(--color-border-tertiary)',color:'var(--color-text-secondary)'}}>✓ {r}</li>)}
        </ul>
        <hr className="divider" />
        {!user
          ? <><p style={{fontSize:13,color:'var(--color-text-secondary)',marginBottom:12}}>Para postularte necesitás crear una cuenta o iniciar sesión.</p><button className="btn btn-primary btn-block" onClick={()=>setDetail({type:'login'})}>Registrarme / Iniciar sesión</button></>
          : yaPostulado
          ...
          return (
            <div key={p.id} className="card">
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                <div><p style={{fontSize:14,fontWeight:500}}>{p.vacantes?.titulo}</p><p style={{fontSize:12,color:'var(--color-text-secondary)'}}>{p.vacantes?.area}</p></div>
                {estadoBadge(p.estado)}
              </div>
              <ul className="timeline">
                {steps.map((st,i)=>{
                  let dc = i<idx?'done':i===idx&&p.estado!=='descartado'?'active':p.estado==='descartado'&&st.key==='descartado'?'rejected':'pending';
                  return <li key={st.key}><div className={`tl-dot ${dc}`}>{dc==='done'?'✓':dc==='active'?'●':dc==='rejected'?'✕':'○'}</div><div><p style={{fontSize:13,fontWeight:500,color:dc==='pending'?'var(--color-text-secondary)':'var(--color-text-primary)'}}>{st.label}</p>{st.key==='entrevista'&&p.fecha_entrevista&&<span style={{fontSize:11,color:'#185A80'}}>{new Date(p.fecha_entrevista).toLocaleString('es-AR')}</span>}</div></li>;
                })}
              </ul>
            </div>
          );
      ...
      <div>
        ...
        {filtradas2.map(v=>(
          <div key={v.id} className="card">
            ...
            <div style={{marginTop:10}}><span style={{fontSize:12,color:'var(--color-text-secondary)'}}>👥 {v.postulantes} postulantes</span></div>
          </div>
        ))}
      </div>
    );
          ...
          <div key={c.id} className="card" style={{cursor:'pointer'}} onClick={()=>setDetail({type:'candidato_hr',data:c})}>
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <p style={{fontSize:12,color:'var(--color-text-secondary)'}}>{c.vacantes?.titulo} · {c.vacantes?.area}</p>
              </div>
    ...
    return (
      <div>
        ...
        <div className="card" style={{marginBottom:12}}>
          <p style={{fontSize:12,color:'var(--color-text-secondary)',marginBottom:8}}>Estado del proceso</p>
        ...
        <div className="card">
          ...
          <textarea value={nota} onChange={e=>setNota(e.target.value)} style={{width:'100%',padding:8,borderRadius:8,border:'0.5px solid var(--color-border-secondary)',fontSize:12,fontFamily:'inherit',minHeight:60,background:'var(--color-background-primary)',color:'var(--color-text-primary)'}} placeholder="Notas privadas del equipo de RRHH..." />
  ...
  function renderCandidato() {
    ...
    if (detail?.type==='exito') return (
      <div style={{textAlign:'center',padding:'52px 16px'}}>
        ...
        <p style={{fontSize:13,color:'var(--color-text-secondary)',marginBottom:24,lineHeight:1.6}}>Recibimos tu CV para <strong>{detail.titulo}</strong>.<br/>Te notificaremos los avances por email.</p>
